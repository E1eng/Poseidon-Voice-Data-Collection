// server.js (Final Version with Static File Serving)
...
async function main() {
  ...
  // --- TAMBAHAN BARU: Izinkan server mengakses file di folder proyek ---
  ...
  wss.on('connection', (ws) => console.log('Browser terhubung!'));
  const fetchDataAndBroadcast = async () => {
    const promises = collections.map(collection => {
      ...
      return fetch(url, options).then(res => res.ok ? res.json() : null).then(data => ({ name: collection.name, count: (data?.data?.[0]?.assetCount) || 0 })).catch(() => ({ name: collection.name, count: 0 }));
    });
